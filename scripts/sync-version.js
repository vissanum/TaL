...
async function updateVersions() {
  try {
    ...
    console.log(
      `Sincronizando a la versión: ${newVersion} (desde ${versionData.version} en src/version.json)`
    )
    ...
    try {
      ...
      if (tauriConfig.version !== newVersion) {
        ...
        console.log(
          `Actualizado ${path.basename(tauriConfigPath)} a la versión ${newVersion}`
        )
      } else {
        console.log(
          `${path.basename(tauriConfigPath)} ya está en la versión ${newVersion}. No se requieren cambios.`
        )
      }
    } catch (e) {
      if (e.code === 'ENOENT') {
        console.warn(
          `ADVERTENCIA: ${tauriConfigPath} no encontrado. Omitiendo actualización. Esto podría ser esperado si se migra a tauri.conf.jsonc.`
        )
      } else {
    ...
    if (packageJson.version !== newVersion) {
      ...
      console.log(
        `Actualizado ${path.basename(packageJsonPath)} a la versión ${newVersion}`
      )
    } else {
      console.log(
        `${path.basename(packageJsonPath)} ya está en la versión ${newVersion}. No se requieren cambios.`
      )
    }
    ...
    const cargoPackageVersionRegex =
      /(\[package\](?:.|\n)*?version\s*=\s*")[^"]*(")/

    if (cargoPackageVersionRegex.test(cargoTomlContent)) {
      const currentCargoVersionMatch = cargoTomlContent.match(
        cargoPackageVersionRegex
      )
      ...
      const currentCargoVersion = currentCargoVersionMatch[0].match(
        /version\s*=\s*"([^"]*)"/
      )[1]
      ...
      if (currentCargoVersion !== newVersion) {
        ...
        console.log(
          `Actualizado ${path.basename(cargoTomlPath)} [package] de la versión ${currentCargoVersion} a ${newVersion}`
        )
      } else {
        console.log(
          `${path.basename(cargoTomlPath)} [package] ya está en la versión ${newVersion}. No se requieren cambios.`
        )
      }
    } else {
      console.warn(
        `ADVERTENCIA: No se encontró la entrada 'version' bajo '[package]' en ${cargoTomlPath}. Omitiendo actualización de Cargo.toml.`
      )
    }
