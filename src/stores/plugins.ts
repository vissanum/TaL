...
export const usePluginsStore = defineStore('plugins', () => {
  ...
  const availableIds = computed(() =>
    installed.value.filter((i) => i.available).map((i) => i.id)
  )
  const [data, ready] = persistentReactive<PluginsData>(
    '#plugins-data',
    defaultData
  )
  const plugins = computed(() => [
    ...
    ...installed.value.map((i) => {
      if (i.type === 'lobechat') return buildLobePlugin(i.manifest, i.available)
        ...
        return buildGradioPlugin(i.manifest, i.available)
  ...
  async function installMcpPlugin(manifest: McpPluginManifest) {
    ...
    await db.transaction(
      ...
      db.reactives,
      async () => {
        const plugin = await db.installedPluginsV2
          .where('id')
          .equals(manifest.id)
          .first()
        if (plugin) {
          await db.installedPluginsV2.update(plugin.key, {
            type: 'mcp',
            available: true,
            manifest: dump,
          })
  ...
  async function uninstall(id) {
    await db.transaction(
      ...
      db.assistants,
      async () => {
        await db.installedPluginsV2
          .where('id')
          .equals(id)
          .modify({ available: false })
        await db.assistants
          .filter((a) => !!a.plugins[id])
          .modify({ [`plugins.${id}`]: undefined })
      }
    )
  }
