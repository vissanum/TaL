...
`,
  ...
  plugins: {
    time: {
      title: '时间和日期',
      description:
        '让 AI 获取当前的时间和日期（没什么用。可用于测试工具调用是否正常）',
      prompt: '获取当前的时间和日期',
    },
    calculator: {
      title: '计算器',
    ...
    flux: {
      title: '图像生成: FLUX',
      description:
        '让 AI 调用 FLUX 模型生成图像。通过 🤗 Spaces 调用，因此是免费的',
    },
    videoTranscript: {
      title: '视频转文字',
      ...
      audioEncoderError:
        '当前浏览器不支持音频编码。建议使用最新版的 Chrome/Edge 浏览器。',
      rangeInput: {
        label: '时间范围',
      },
    },
    ...
    mermaid: {
      ...
      prompt:
        '在回答中，如果需要绘制图表，你可以直接使用 mermaid 语法创建图表，它们能够被正常渲染。',
    },
    mcp: {
      runCommand: '运行命令',
      cwd: '工作目录',
  ...
  webSearchPlugin: {
    ...
    configureSearxngMessage:
      '请在服务端配置 SearXNG 实例，或在插件设置中配置 SearXNG URL',
  },
  docParsePlugin: {
    title: '文档解析',
    description:
      '解析文档（PDF、Word、Excel、PPT 等）内容，并转换为 Markdown 文本',
    ...
    pagesLimitDialog: {
      title: '页数过多',
      message:
        '当前要渲染的文档截图数量过多（>{limit}），过多的图片可能导致加载缓慢，并影响模型的理解效果。建议指定页码范围，重新解析。是否继续解析？',
      cancel: '取消',
      continue: '继续解析',
    },
    emptyPagesDialog: {
      title: '空页过多',
      message:
        "超过一半的页面解析结果为空，这可能是因为该 PDF 文档由扫描图片构成，而此解析器只能提取文档中的文字。建议使用「@:{'docParsePlugin.pdfScreenshotLabel'}」解析器重新解析。",
    },
  },
}
