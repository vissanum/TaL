<template>
  <template v-if="workspace">
    ...
    >
      <dragable-separator
        ...
        w-2
      />
      <div v-if="showArtifacts" h-full min-w-0 flex="~ col 1">
        <div flex items-center h="50px">
          <q-tabs inline-label dense mt="14px" rd-t>
            <q-route-tab
              no-caps
            ...
            >
              ...
              <div v-if="artifactUnsaved(artifact)">*</div>
              <q-btn
                ml-1
                flat
                dense
        ...
        <edit-artifact :artifact="focusedArtifact" v-if="focusedArtifact" />
      </div>
      <div w="250px" h-full flex="~ col">
        <div h="48px" p-2 flex items-center>
          <q-space />
          <q-btn
            flat
            dense
          ...
          <q-btn
            ...
            :class="{
              'route-active': $route.path === `/workspaces/${id}/settings`,
            }"
  ...
  <error-not-found v-else drawer-toggle />
</template>
...
const workspace = computed(
  () =>
    workspacesStore.workspaces.find((item) => item.id === props.id) as Workspace
)
const dialogs = useLiveQueryWithDeps(
  () => props.id,
  () => db.dialogs.where('workspaceId').equals(props.id).toArray(),
  { initialValue: [] as Dialog[] }
)
const artifacts = useLiveQueryWithDeps(
  () => props.id,
  () => db.artifacts.where('workspaceId').equals(props.id).toArray(),
  { initialValue: [] as Artifact[] }
)
