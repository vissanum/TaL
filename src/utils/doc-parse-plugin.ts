  ...
  if (!targetPages && pages.length > pagesLimit) {
    const continueRender = await new Promise((resolve) => {
      Dialog.create({
        ...
        message: t('docParsePlugin.pagesLimitDialog.message', {
          limit: pagesLimit,
        }),
...
async function parsePptx(
  ...
  const slideFileNames = Object.keys(files)
  ...
  return [
    {
      type: 'text',
      contentText: (targetPages
        ? targetPages.map((i) => texts[i])
        : texts
      ).join('\n\n---\n\n'),
    },
  ]
  ...
  fileparsers: [
    {
      ...
      async execute({ file, range }) {
        return await extractPdfText(file, {
          targetPages: range ? parsePageRange(range) : undefined,
        })
      },
      rangeInput: {
    ...
    },
    {
      ...
      async execute({ file, range }) {
        return await renderPdfImages(file, {
          targetPages: range ? parsePageRange(range) : undefined,
        })
      },
      rangeInput: {
